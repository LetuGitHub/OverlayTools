...
const char *ovl_origin_xattr   = "trusted.overlay.origin";
const char *ovl_impure_xattr   = "trusted.overlay.impure";
// should set index=off
// const char *ovl_nlink_xattr = "trusted.overlay.nlink";
...
const char *ftype_name(mode_t type) {
    switch (type) {
        ...
        default: return "special file";
    }
}

const char *ftype_name_plural(mode_t type) {
    switch (type) {
        case S_IFDIR: return "Directories";
        case S_IFREG: return "Files";
        case S_IFLNK: return "Symbolic links";
        default: return "Special files";
    }
}
...
permission_bits(const struct stat *
                    status) { // not used yet. I haven't decided how to treat permission bit changes
...
int is_opaque(const char *path, bool *output) {
    ...
    if ((res < 0) && (errno != ENODATA)) { return -1; }
    *output = (res == 1 && val == 'y');
    return 0;
}
...
int is_redirect_match(const char *path, bool *output) {
    ssize_t res = getxattr(path, ovl_redirect_xattr, NULL, 0);
    if ((res < 0) && (errno != ENODATA)) {
        fprintf(stderr, "File %s redirect xattr can not be read.\n", path);
        return -1;
...
int is_origin(const char *path, bool *output) {
    ssize_t res = getxattr(path, ovl_origin_xattr, NULL, 0);
    if ((res < 0) && (errno != ENODATA)) {
        fprintf(stderr, "File %s origin xattr can not be read.\n", path);
        return -1;
    }
    *output = (res >= 0);
    return 0;
...
int is_impure(const char *path, bool *output) {
    char    val;
    ssize_t res = getxattr(path, ovl_impure_xattr, &val, 1);
    if ((res < 0) && (errno != ENODATA)) { return -1; }
    *output = (res == 1 && val == 'y');
    return 0;
}
...
bool permission_identical(const struct stat *lower_status, const struct stat *upper_status) {
    return (permission_bits(lower_status) == permission_bits(upper_status))
...
int regular_file_identical(const char *lower_path, const struct stat *lower_status,
                           const char *upper_path, const struct stat *upper_status, bool *output) {
    ...
    ssize_t read_lower;
    ssize_t read_upper;
    do { // we can assume one will not reach EOF earlier than the other, as the file sizes are
         // checked to be the same earlier
        ...
        if (memcmp(lower_buffer, upper_buffer, read_upper)) {
            *output = false;
            return 0;
        } // the output is by default false, but we still set it for ease of reading
...
static int vacuum_d(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                    const struct stat *lower_status, const struct stat *upper_status,
                    FILE *script_stream, int *fts_instr) {
    bool opaque;
    if (is_opaquedir(upper_path, &opaque) < 0) { return -1; }
    if (opaque) { // TODO: sometimes removing opaque directory (and combine with lower directory)
                  // might be better
        *fts_instr = FTS_SKIP;
    }
    return 0;
...
static int vacuum_dp(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                     const struct stat *lower_status, const struct stat *upper_status,
                     FILE *script_stream, int *fts_instr) {
    ...
    if (is_opaquedir(upper_path, &opaque) < 0) { return -1; }
    if (opaque) { return 0; }
    // this directory might be empty if all children are deleted in previous commands. but we simply
    // don't test whether it's that case
    return command(script_stream, "rmdir --ignore-fail-on-non-empty %U", upper_path);
}

static int vacuum_f(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                    const struct stat *lower_status, const struct stat *upper_status,
                    FILE *script_stream, int *fts_instr) {
    if (lower_status == NULL) { return 0; } // lower does not exist
    ...
    if (!permission_identical(lower_status, upper_status)) { return 0; }
    bool identical;
    if (regular_file_identical(lower_path, lower_status, upper_path, upper_status, &identical)
        < 0) {
        return -1;
    }
...
static int vacuum_sl(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                     const struct stat *lower_status, const struct stat *upper_status,
                     FILE *script_stream, int *fts_instr) {
    if (lower_status == NULL) { return 0; } // lower does not exist
    ...
    if (!permission_identical(lower_status, upper_status)) { return 0; }
    bool identical;
    if (symbolic_link_identical(lower_path, upper_path, &identical) < 0) { return -1; }
    if (!identical) { return 0; }
    return command(script_stream, "rm %U", upper_path);
}
...
void print_added(const char *lower_path, const size_t lower_root_len, const char *upper_path,
                 mode_t upper_type) {
    if (brief) {
...
void print_replaced(const char *lower_path, const size_t lower_root_len, mode_t lower_type,
                    const char *upper_path, mode_t upper_type) {
    if (brief) {
        printf("File %s is a %s while file %s is a %s\n", lower_path, ftype_name(lower_type),
               upper_path, ftype_name(upper_type));
    } else {
...
void print_modified(const char *lower_path, const size_t lower_root_len, mode_t lower_type,
                    const char *upper_path, bool identical) {
    if (brief) {
...
int list_deleted_files(
    const char *lower_path, size_t lower_root_len,
    mode_t upper_type) { // This WORKS with files and itself is listed. However, prefixs are WRONG!
    ...
    while (((cur = fts_read(ftsp)) != NULL) && (return_val == 0)) {
        switch (cur->fts_info) {
            ...
            case FTS_DP:
                ...
                if (!children) { print_removed(cur->fts_path, lower_root_len, S_IFDIR); }
                break;
            case FTS_F: print_removed(cur->fts_path, lower_root_len, S_IFREG); break;
            case FTS_SL: print_removed(cur->fts_path, lower_root_len, S_IFLNK); break;
            case FTS_DEFAULT:
                fprintf(stderr,
                        "File %s is a special file (device or pipe). We cannot handle that.\n",
                        cur->fts_path);
                return_val = -1;
                break;
            default:
    ...
    if (errno) {
        return_val = -1;
    } // if no error happened, fts_read will "sets the external variable errno to 0" according to
      // the documentation
    return fts_close(ftsp) || return_val;
}

static int diff_d(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                  const struct stat *lower_status, const struct stat *upper_status,
                  FILE *script_stream, int *fts_instr) {
    bool opaque      = false;
    ...
    if (lower_exist) {
        if (file_type(lower_status) == S_IFDIR) {
            ...
            if (opaque) {
                ...
                return 0; // children must be recursed, and directory itself does not need to be
                          // printed
            }
        } else { // other types of files
            print_replaced(lower_path, lower_root_len, file_type(lower_status), upper_path,
                           S_IFDIR);
        }
    }
    ...
    if (!lower_exist
        || (!brief && opaque)) { // brief format does not need to print opaque dir itself
        print_added(lower_path, lower_root_len, upper_path, S_IFDIR);
    }
    return 0;
}

static int diff_f(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                  const struct stat *lower_status, const struct stat *upper_status,
                  FILE *script_stream, int *fts_instr) {
    bool identical;
    if (lower_status != NULL) {
        switch (file_type(lower_status)) {
            case S_IFREG:
                if (regular_file_identical(lower_path, lower_status, upper_path, upper_status,
                                           &identical)
                    < 0) {
                    return -1;
                }
...
static int diff_sl(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                   const struct stat *lower_status, const struct stat *upper_status,
                   FILE *script_stream, int *fts_instr) {
    bool identical;
    if (lower_status != NULL) {
...
static int diff_whiteout(const char *lower_path, const char *upper_path,
                         const size_t lower_root_len, const struct stat *lower_status,
                         const struct stat *upper_status, FILE *script_stream, int *fts_instr) {
    if (lower_status != NULL) {
        if (file_type(lower_status) == S_IFDIR) {
...
static int merge_d(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                   const struct stat *lower_status, const struct stat *upper_status,
                   FILE *script_stream, int *fts_instr) {
    bool redirect;
    if (is_redirect(upper_path, &redirect) < 0) { return -1; }
    // merging redirects is not supported, we must abort merge so redirected lower (under whiteout)
    // won't be deleted upper_path may be hiding the directory in lower_path, but there may be
    // another redirect upper pointing at it
    if (redirect) {
...
static int merge_dp(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                    const struct stat *lower_status, const struct stat *upper_status,
                    FILE *script_stream, int *fts_instr) {
    if (lower_status != NULL) {
        if (file_type(lower_status) == S_IFDIR) {
...
static int merge_f(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                   const struct stat *lower_status, const struct stat *upper_status,
                   FILE *script_stream, int *fts_instr) {
    bool metacopy, redirect;
    if (is_metacopy(upper_path, &metacopy) < 0) { return -1; }
    ...
    if (metacopy || redirect) {
        fprintf(
            stderr,
            "Found metacopy/redirect on %s. Merging metacopy/redirect is not supported - Abort.\n",
            upper_path);
        return -1;
    }
    return command(script_stream, "rm -rf %L", lower_path)
           || command(script_stream, "mv -T %U %L", upper_path, lower_path);
}

static int merge_sl(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                    const struct stat *lower_status, const struct stat *upper_status,
                    FILE *script_stream, int *fts_instr) {
    return command(script_stream, "rm -rf %L", lower_path)
           || command(script_stream, "mv -T %U %L", upper_path, lower_path);
...
static int merge_whiteout(const char *lower_path, const char *upper_path,
                          const size_t lower_root_len, const struct stat *lower_status,
                          const struct stat *upper_status, FILE *script_stream, int *fts_instr) {
    return command(script_stream, "rm -r %L", lower_path)
           || command(script_stream, "rm %U", upper_path);
}


static int nlo_d(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                 const struct stat *lower_status, const struct stat *upper_status,
                 FILE *script_stream, int *fts_instr) {
    // lower not dir
    if (lower_status != NULL && file_type(lower_status) != S_IFDIR) {
        *fts_instr = FTS_SKIP;
        return command(script_stream, "rm -rf %U", upper_path);
    }
    bool flag = false;
    // ignore redirect, or you can use fsck.overlay check
    // if (is_redirect(upper_path, &flag) < 0) { return -1; }
    // if (flag) {

    // }
    if (is_opaque(upper_path, &flag) < 0) { return -1; }
    if (flag) {
        if (command(script_stream, "setfattr -x trusted.overlay.opaque %U", upper_path) < 0) {
            return -1;
        };
    }
    ...
    } else { // lower not exists
        // ignore child files, retain it
        *fts_instr = FTS_SKIP;
        if (is_impure(upper_path, &flag) < 0) { return -1; }
        if (flag) {
            if (command(script_stream, "setfattr -x trusted.overlay.impure %U", upper_path) < 0) {
                return -1;
            };
        }
        if (is_origin(upper_path, &flag) < 0) { return -1; }
        if (flag) {
            return command(script_stream, "setfattr -x trusted.overlay.origin %U", upper_path);
        }
    }
    return 0;
}

static int nlo_dp(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                  const struct stat *lower_status, const struct stat *upper_status,
                  FILE *script_stream, int *fts_instr) {
    // lower exists
    if (lower_status != NULL) {
...
static int nlo_f(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                 const struct stat *lower_status, const struct stat *upper_status,
                 FILE *script_stream, int *fts_instr) {
    if (lower_status != NULL) return command(script_stream, "rm -f %U", upper_path);

    bool flag;
    if (is_metacopy(upper_path, &flag) < 0) { return -1; }
    // delete metacopy file and origin attr
    if (flag) { return command(script_stream, "rm -f %U", upper_path); }
    if (is_origin(upper_path, &flag) < 0) { return -1; }
    if (flag) {
        return command(script_stream, "setfattr -x trusted.overlay.origin %U", upper_path);
    } else
...
static int nlo_sl(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                  const struct stat *lower_status, const struct stat *upper_status,
                  FILE *script_stream, int *fts_instr) {
    if (lower_status != NULL) return command(script_stream, "rm -f %U", upper_path);

    // symbol link file do not check attr
...
static int nlo_whiteout(const char *lower_path, const char *upper_path, const size_t lower_root_len,
                        const struct stat *lower_status, const struct stat *upper_status,
                        FILE *script_stream, int *fts_instr) {
    return command(script_stream, "rm -f %U", upper_path);
}


typedef int (*TRAVERSE_CALLBACK)(const char *lower_path, const char *upper_path,
                                 const size_t lower_root_len, const struct stat *lower_status,
                                 const struct stat *upper_status, FILE *script_stream,
                                 int *fts_instr);

int traverse(const char *lower_root, const char *upper_root, FILE *script_stream,
             TRAVERSE_CALLBACK callback_d, TRAVERSE_CALLBACK callback_dp,
             TRAVERSE_CALLBACK callback_f, TRAVERSE_CALLBACK callback_sl,
             TRAVERSE_CALLBACK callback_whiteout) { // returns 0 on success
    FTSENT *cur;
    ...
    while ((return_val == 0) && ((cur = fts_read(ftsp)) != NULL)) {
        ...
        switch (cur->fts_info) {
            case FTS_D: callback = callback_d; break;
            case FTS_DP: callback = callback_dp; break;
            case FTS_F: callback = callback_f; break;
            case FTS_SL: callback = callback_sl; break;
            case FTS_DEFAULT:
        ...
        if (callback != NULL) {
            ...
            if (lstat(lower_path, &lower_status) != 0) {
                if (errno == ENOENT || errno == ENOTDIR) { // the corresponding lower file (or its
                                                           // ancestor) does not exist at all
                    lower_exist = false;
            ...
            return_val = callback(lower_path, cur->fts_path, lower_root_len,
                                  lower_exist ? &lower_status : NULL, cur->fts_statp, script_stream,
                                  &fts_instr); // return_val must previously be 0
            if (fts_instr) { fts_set(ftsp, cur, fts_instr); }
        }
    }
    if (errno) {
        return_val = -1;
...
static int deref_d(const char *mnt_path, const char *upper_path, const size_t mnt_root_len,
                   const struct stat *mnt_status, const struct stat *upper_status,
                   FILE *script_stream, int *fts_instr) {
    bool redirect;
    if (is_redirect(upper_path, &redirect) < 0) { return -1; }
    ...
    *fts_instr = FTS_SKIP;
    return command(script_stream, "rm -rf %U", upper_path)
           || command(script_stream, "cp -a %M %U", mnt_path, upper_path);
}

static int deref_f(const char *mnt_path, const char *upper_path, const size_t mnt_root_len,
                   const struct stat *mnt_status, const struct stat *upper_status,
                   FILE *script_stream, int *fts_instr) {
    bool metacopy;
    if (is_metacopy(upper_path, &metacopy) < 0) { return -1; }
    if (!metacopy) { return 0; }
    return command(script_stream, "rm -r %U", upper_path)
           || command(script_stream, "cp -a %M %U", mnt_path, upper_path);
}

int vacuum(const char *lowerdir, const char *upperdir, FILE *script_stream) {
    return traverse(lowerdir, upperdir, script_stream, vacuum_d, vacuum_dp, vacuum_f, vacuum_sl,
                    NULL);
}
...
int merge(const char *lowerdir, const char *upperdir, FILE *script_stream) {
    return traverse(lowerdir, upperdir, script_stream, merge_d, merge_dp, merge_f, merge_sl,
                    merge_whiteout);
}
...
int new_lower_overwrite(const char *lowerdir, const char *upperdir, FILE *script_stream) {
    return traverse(lowerdir, upperdir, script_stream, nlo_d, nlo_dp, nlo_f, nlo_sl, nlo_whiteout);
}