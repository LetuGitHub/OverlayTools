...
void print_help(const char *program) {
    ...
    puts(
        "  nlo    - new lower overwrite, delete upper files that exists both on upper and lower and all whiteout files.");
    puts("           remove opaque, metacopy, impure, origin attr");
    puts("");
    puts("Options:");
...
bool is_mounted(const char *lower, const char *upper) {
    ...
    while (fgets(buf, STRING_BUFFER_SIZE, f)) {
        if (!starts_with(buf, "overlay")) { continue; }
...
//         printf("It is strongly recommended to unmount OverlayFS first. Still continue (not
//         recommended)?: \n"); int r = getchar(); if (r != 'Y' && r != 'y') {
//             return true;
//         }
//     }
//     return false;
...
int main(int argc, char *argv[]) {
    ...
    static struct option long_options[] = { { "lowerdir", required_argument, 0, 'l' },
    ...
    while ((opt = getopt_long_only(argc, argv, "l:u:m:L:U:hvb", long_options, &long_index)) != -1) {
        switch (opt) {
            ...
            case 'b':
                ...
                brief   = true;
                break;
            default: fprintf(stderr, "Option %c is not supported.\n", opt); goto see_help;
        }
    }
    ...
    if (optind == argc - 1) {
        int   out;
        char  filename_template[] = "/tmp/overlay-tools-XXXXXX.sh";
        FILE *script              = NULL;
        if (strcmp(argv[optind], "diff") == 0) {
            ...
            if (script == NULL) {
                fprintf(stderr, "Script file cannot be created.\n");
                return EXIT_FAILURE;
            }
        ...
        } else if (strcmp(argv[optind], "nlo") == 0) {
            script = create_shell_script(filename_template);
            if (script == NULL) {
                fprintf(stderr, "Script file cannot be created.\n");
                return EXIT_FAILURE;
            }
            out = new_lower_overwrite(lower, upper, script);
        } else if (strcmp(argv[optind], "deref") == 0) {
            if (!mnt || !vars[UPPERNEW]) {
                fprintf(stderr, "'deref' command requires --uppernew and --mountdir.\n");
                return EXIT_FAILURE;
            }
        ...
        if (script != NULL) {
            printf(
                "The script is created. Run the script to do the actual work please. Remember to run it when the OverlayFS is not mounted.\n%s",
                filename_template);
            fclose(script);
        }
        if (out) {
